...
def append_level_to_columns_of_dataframe(dataframe: pd.DataFrame,
                                         new_level,
    ...
    Args:
        dataframe (pd.DataFrame): a DataFrame with Index or MultiIndex columns
        new_level: scalar, or arraylike of length equal to the number of columns
            in `dataframe`. The labels to put on the columns. If scalar, it is
            broadcast into a list of length equal to the number of columns in
            `dataframe`.
        name_of_new_level (str): The label to give the new level.
        inplace (bool, optional): Whether to modify `dataframe` in place or to
            return a copy that is modified. Defaults to False.

    Returns:
        dataframe_with_new_columns (pd.DataFrame): DataFrame with MultiIndex
            columns. The original `dataframe` with new columns that have the
            given `level` appended to each column label.
    '''
