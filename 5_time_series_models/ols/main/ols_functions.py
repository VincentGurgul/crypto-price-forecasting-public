''' Functions for forecasting crypto prices with (Ridge) OLS / Logistic
Regression with optimal hyperparameters. '''
...
def run_and_log_ols(mode: str,
    ...
    if mode in ['causal_stationary_no_nlp', 'causal_nonstationary']:
        ...
        X = X[columns_to_keep]
    elif mode == 'causal_stationary_full_data':
        X = data[price_log_difference_vars]
    else:
        columns_to_keep = [
